  ...
  <main class="container bg-[#EDEDED] p-5">
    <div v-if="animeStore.anime">
      ...
      <div class="flex items-start max-[981px]:flex-wrap">
        ...
        >
          <img
            class="block rounded-lg w-full max-h-[250px] max-[981px]:max-h-full"
            v-lazy="{
              src: animeStore.animePoster,
              loaded: animeStore.animePoster,
              loading: animeStore.imagePlaceholder,
              error: animeStore.imagePlaceholder,
            }"
            alt="Poster"
          />
          <span
          ...
          <div
            class="absolute bottom-0 left-0 right-0 bg-gray-600 min-h-10 rounded-b-lg rounded-bl-lg"
          >
            <div
              class="w-full max-w-[70px] px-2.5 py-3 bg-white rounded-[50%] text-xl mx-auto font-bold absolute -top-[30px] left-[50%] -translate-x-[50%] border-4 border-green-700 text-center"
            >
              {{ animeStore.animeScore }}
            </div>
          </div>
          <base-favorite-btn
            v-if="firebaseUserStore.user"
            :is-in-favorites="animeRouteStore.isAnimeFavorite(animeId)"
          ...
          <div class="max-w-[900px] mb-[15px]">
            ...
            <ul>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Год выпуска:</span> {{ animeStore.animeYear }}
              </li>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Жанр:</span> {{ animeStore.animeGenres }}
              </li>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Категория:</span> {{ animeStore.animeType }}
              </li>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Всего серий:</span> {{ animeStore.animeEpisodes }}
              </li>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Студия:</span> {{ animeStore.animeStudios }}
              </li>
              <li class="mb-2 text-xl">
                <span class="font-bold text-lg">Режиссёр:</span> {{ animeStore.animeProducers }}
              </li>
            </ul>
            <p class="text-lg text-[#888] mb-5">
              <span class="text-lg font-bold text-[#444]">Описание:</span>
              {{ animeStore.animeDescription }}
            </p>
            <a
              v-if="animeStore.animeExtendedInfo"
              target="_blank"
              :href="animeStore.animeExtendedInfo"
              class="text-cyan-600 font-bold text-lg"
              >Подробнее здесь</a
            >
          </div>
        </div>
